...
if [ ! -f "/opt/wso2am/repository/conf/initialized" ]; then
  ...
  saxonb-xslt -s:/opt/wso2am/repository/conf/tomcat/catalina-server.xml -xsl:/tmp/catalina-server.xsl -o:/opt/wso2am/repository/conf/tomcat/catalina-server.xml password=$PRIVATE_KEY_PASSWORD proxyport=$PROXY_PORT
  rm /tmp/*.xsl
